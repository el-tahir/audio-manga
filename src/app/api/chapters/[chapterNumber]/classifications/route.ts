...
export async function GET(
  ...
  try {
    ...
    const { data: chapterExists, error: chapErr } = await supabase
      ...
      .maybeSingle();
    if (chapErr) {
      console.error('[API /api/chapters/[chapterNumber]/classifications GET] Chapter query error:', chapErr);
    ...
    if (error) {
      console.error(
        '[API /api/chapters/[chapterNumber]/classifications GET] DB error:',
        error
      );
      return NextResponse.json(
        { error: error.message },
