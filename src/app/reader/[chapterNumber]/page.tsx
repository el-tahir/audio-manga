...
import { PageParams, Page, ClassificationData, ChapterStatus } from '@/types';
...
export default async function ChapterReaderPage({ params }: PageParams) {
  ...
  try {
    ...
    if (chapterError && chapterError.code !== 'PGRST116') {
      // Ignore 'No rows found' for single()
    ...
    if (nextChapterError && nextChapterError.code !== 'PGRST116') {
      // Ignore 'No rows found' for maybeSingle()
    ...
    if (totalPages > 0) {
      ...
      const urlPromises = Array.from({ length: totalPages }, (_, i) => i + 1).map(
        async pageNumber => {
          try {
    ...
    <div className="min-h-screen bg-bg-primary text-foreground flex flex-col">
      ...
      <main className="flex-1 container mx-auto p-4 md:p-6 lg:p-8 max-w-4xl">
        ...
        <div className="mb-6 md:mb-8 flex items-center justify-between">
          ...
          {chapterData && (
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-center text-white flex-1 px-4">
              {' '}
          ...
          <div className="w-auto min-w-[150px]">
            {' '}
            {/* Adjust min-width as needed based on button size */}
