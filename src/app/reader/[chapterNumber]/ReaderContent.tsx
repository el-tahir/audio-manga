...
export default function ReaderContent({
  ...
  const [overallError, setOverallError] = useState<string | null>(initialOverallError);
  ...
  const handleNextChapterClick = async () => {
    ...
    if (currentStatus === 'completed') {
      ...
      setIsLoading(true);
      const optimisticStatus: ChapterStatus = currentStatus === 'failed' ? 'pending' : 'downloading';
      ...
      try {
        ...
        if (response.ok) {
            if (response.status === 202) {
                setStatusMessage(`Chapter ${nextChapterNumber} download started. Processing in background.`);
                setCurrentStatus('pending');
            } else if (response.status === 409) {
                 setStatusMessage(result.message || `Chapter ${nextChapterNumber} already processing/available.`);
                 // Attempt to determine a more specific status if possible from result
                 // For now, stick with pending as a safe bet
                 setCurrentStatus('pending'); 
            } else {
                setStatusMessage(result.message || `Request acknowledged for chapter ${nextChapterNumber}.`);
                setCurrentStatus('pending');
            }
            setIsError(false);
        } else {
          setIsError(true);
          setStatusMessage(`Error: ${result.error || `Failed to start download (Status: ${response.status})`}`);
  ...
  return (
    <>
      ...
      <div className="mb-6 md:mb-8 flex items-center justify-between">
         ...
         {chapterData && (
           <div className="w-auto"> {/* Adjust width as needed */} 
      ...
      {!overallError && chapterData && (
        <div className="flex flex-col items-center gap-4 max-w-2xl mx-auto pb-16"> {/* Reduced pb slightly */} 
          {pages.map((page) => (
          ...
          {pages.length > 0 && (
            <div className="mt-8 text-center"> {/* Center the bottom button */} 
              <NextChapterButton {...nextButtonProps} />
